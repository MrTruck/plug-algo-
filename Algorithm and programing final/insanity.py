...
choices = [True, True,True, True, True, True, True] #it's suppose to be the choice for letter length, refer to generate_level() for how it works
...
def generate_level():
    ...
    speed = random.randint(2 , 3  )#speed is random either 2 or 3 but increases as the level increases though i added //3 bcause it got too fast (thanks to sir jude for the idea)
    ...
    index = len_indexes#for picking from wordlist using index with the range of the tuples from index_sel
    text = wordlist[54:100]#this is why wordlist needed to be sorted
...
while run:
    ...
    if not paused: #if new_level is true and its not paused generate the words
